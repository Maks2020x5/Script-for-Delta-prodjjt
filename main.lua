...
local function getAllCharacters()
    ...
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj:IsA("Model") and obj ~= currentCharacter then
...
local function getBestVisibleBone(character)
    ...
    local ignoreList = {lPlr.Character, cam, character}
    if workspace:FindFirstChild("Loot") then table.insert(ignoreList, workspace.Loot) end
    ...
    if getgenv().aimbotTargetMode == "Head" then
        if isR15 then bonePriorityOrder = {"Head", "UpperTorso", "LowerTorso", "LeftHand", "RightHand", "LeftLowerArm", "RightLowerArm"}
        else bonePriorityOrder = {"Head", "Torso", "Left Arm", "Right Arm"} end
    else
        if isR15 then bonePriorityOrder = {"UpperTorso", "LowerTorso", "Head", "LeftHand", "RightHand", "LeftLowerArm", "RightLowerArm"}
        else bonePriorityOrder = {"Torso", "Head", "Left Arm", "Right Arm"} end
    end
    for _, boneName in ipairs(bonePriorityOrder) do
        ...
        if part then
            ...
            if not cast then wallCheckCache[character] = part return part end
        end
    end
...
thermalActionButton.MouseButton1Click:Connect(function()
    ...
    if thermalActive then
        if nvgActive then nvgActive = false; actionButton.BackgroundColor3 = Color3.fromRGB(30, 30, 30); actionButton.TextColor3 = Color3.fromRGB(0, 255, 0) end
...
local function updateHitboxes()
    if not getgenv().hitboxExpanderEnabled then
        for part, data in pairs(originalHitboxes) do if part and part.Parent then part.Size = data.Size; part.Transparency = data.Transparency; part.Color = data.Color; part.CanCollide = data.CanCollide end end
        table.clear(originalHitboxes) return
    end
        ...
        if char and char:FindFirstChildOfClass("Humanoid") and char:FindFirstChildOfClass("Humanoid").Health > 0 then
            ...
            for _, name in ipairs(targetParts) do
                ...
                if part and part:IsA("BasePart") then
                    if not originalHitboxes[part] then originalHitboxes[part] = {Size = part.Size, Transparency = part.Transparency, Color = part.Color, CanCollide = part.CanCollide} end
                    part.Size = originalHitboxes[part].Size * 3; part.Transparency = 0.6; part.Color = Color3.fromRGB(0, 170, 255); part.CanCollide = false
                end
            end
        end
    end
...
local function createToggle(text, env_val, order)
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.new(0, 180, 0, 26); btn.Font = Enum.Font.SourceSansBold; btn.TextSize = 12; btn.BorderSizePixel = 0; btn.LayoutOrder = order; btn.Parent = contentFrame
...
local function setupLocationsESP()
    ...
    for _, loc in ipairs(mapLocations) do
        ...
        nPart.Name = "Loc_" .. loc.Name; nPart.Position = loc.Pos; nPart.Anchored = true; nPart.CanCollide = false; nPart.Transparency = 1; nPart.Size = Vector3.new(1, 1, 1); nPart.Parent = locFolder
        local b = Instance.new("BillboardGui", nPart); b.Name = "LocGui"; b.AlwaysOnTop = true; b.MaxDistance = 10000; b.Size = UDim2.new(0, 160, 0, 28)
        local t = Instance.new("TextLabel", b); t.Size = UDim2.new(1, 0, 1, 0); t.BackgroundTransparency = 0.2; t.BackgroundColor3 = Color3.fromRGB(15, 15, 30); t.TextSize = 12; t.Font = Enum.Font.SourceSansBold; t.TextColor3 = Color3.fromRGB(255, 255, 255)
        local stroke = Instance.new("UIStroke", t); stroke.Color = Color3.fromRGB(255, 255, 0); stroke.Thickness = 1.5
        local corner = Instance.new("UICorner", t); corner.CornerRadius = UDim.new(0, 6)
    end
end
setupLocationsESP()
runService.RenderStepped:Connect(function()
    ...
    if getgenv().aimbotEnabled and (os.clock() - lastAimTime > 0.01) then
        ...
        if targetPart and fovDist and targetPart.Parent ~= lPlr.Character then 
            ...
            if enemyHrp and myHrp then 
                local bulletSpeed = 1200; local distance = (myHrp.Position - targetPart.Position).Magnitude
                local travelTime = distance / bulletSpeed; local movePrediction = enemyHrp.Velocity * (travelTime * 0.7)
                local gravityCorrection = Vector3.new(0, 0.5 * 40 * (travelTime ^ 2), 0) 
            ...
            for _, child in ipairs(locFolder:GetChildren()) do
                ...
                if gui then
                    ...
                    if getgenv().locationsEspEnabled then
                        local dist = math.floor((myPos - child.Position).Magnitude); local cleanName = string.gsub(child.Name, "Loc_", "")
                ...
                if (isLoot and getgenv().itemEspEnabled) or (isMine and getgenv().mineEspEnabled) or (isDrop and getgenv().itemEspEnabled) then
                    ...
                    if p then
                        ...
                        if dist <= 500 then 
                            ...
                            if not b then
                                b = Instance.new("BillboardGui", p); b.Name = guiName; b.AlwaysOnTop = true; b.MaxDistance = 500; b.Size = UDim2.new(0, 140, 0, 25); b.StudsOffset = Vector3.new(0, 2, 0)
                            ...
                            if isMine then cleanName = "💥 МИНА"; b.TextLabel.TextColor3 = Color3.fromRGB(255, 50, 50)
