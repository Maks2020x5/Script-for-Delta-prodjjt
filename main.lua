...
local function makeButtonDraggable(button)
    ...
    inputService.InputChanged:Connect(function(input)
        if input == bDragInput and bDragging then
            ...
            button.Position = UDim2.new(bStartPos.X.Scale, bStartPos.X.Offset + delta.X, startPos.Y.Scale, bStartPos.Y.Offset + delta.Y)
        end
    end)
end
    ...
    local function updateVisuals()
        if getgenv()[env_val] then
            ...
            if env_val == "thermalButtonEnabled" then 
                thermalActionButton.Visible = false; thermalActive = false; thermalActionButton.BackgroundColor3 = Color3.fromRGB(30, 30, 30); resetLightingEffects()
            end
...
local t = Instance.new("TextLabel", b); t.Size = UDim2.new(1, 0, 1, 0); t.BackgroundTransparency = 0.3; t.BackgroundColor3 = Color3.fromRGB(20, 20, 20); t.TextSize = 11; t.Font = Enum.Font.SourceSansBold; Instance.new("UICorner", t).CornerRadius = UDim.new(0, 6)endlocal cleanName = "📦 ОБЪЕКТ"if isMine then cleanName = "💥 МИНА / РАСТЯЖКА"; b.TextLabel.TextColor3 = Color3.fromRGB(255, 50, 50)elseif isDrop then cleanName = "✈️ АИРДРОП"; b.TextLabel.TextColor3 = Color3.fromRGB(0, 255, 255)elseif string.find(nameLower, "^aurora") then cleanName = "✨ АВРОРА КЕЙС"; b.TextLabel.TextColor3 = Color3.fromRGB(255, 215, 0)elseif string.find(nameLower, "^safe") or string.find(nameLower, "^сейф") then cleanName = "🗄️ СЕЙФ"; b.TextLabel.TextColor3 = Color3.fromRGB(255, 165, 0)else cleanName = "🎁 КЕЙС / ЯЩИК"; b.TextLabel.TextColor3 = Color3.fromRGB(0, 255, 100) endb.TextLabel.Text = cleanName .. " [" .. tostring(dist) .. "m]"elseif p:FindFirstChild("LootTextGui") then p.LootTextGui:Destroy() endif p:FindFirstChild("MineTextGui") then p.MineTextGui:Destroy() endendendendendendtask.wait(1.0)endend)for _, v in pairs(workspace:GetDescendants()) do pcall(function() applyLightESP(v) end) end
